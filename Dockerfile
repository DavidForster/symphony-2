...
# Update packages, Install libxslt, zlib and Git, then clean up
RUN apt-get update \
    && apt-get install -y \
        git \
        libxslt1-dev \
...
# Add the standard submodules and workspace, then change the owner and group of all files to www-data
